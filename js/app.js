...
onEnter["ch-name"] = () => {
  ...
  async function finishName() {
    ...
    await sleep(2400); $("#nameNext").hidden = false;           // wait for the slow rise-up to finish before showing Next
  }
};
...
async function tryThreeCake(host) { if (REDUCED) return false; try { const m = await import("./cake3d.js?v=11"); await m.mountCake3D(host, "AMEENA"); return true; } catch (_) { return false; } }
